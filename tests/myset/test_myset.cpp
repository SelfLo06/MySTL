...
namespace TestMySet {
    ...
    // --- 注册所有测试用例 ---
    static const std::vector<TestCase> myset_test_cases = {
    ...
    const std::vector<TestCase>& get_test_cases() {
        return myset_test_cases;
    }

    void run_all_tests() {
        TestRunner::print_separator("MySet Tests");
        for (const auto& test_case : myset_test_cases) {
            TestRunner::run_test(test_case.name, test_case.function);
        }
        TestRunner::print_summary();
        TestRunner::print_separator("MySet Tests Complete");
    }
