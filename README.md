...
##### **15. `MyDisjointSet` (并查集)**
*   **定位:** 一个用于高效管理**不相交集合**（Disjoint Sets）的、支持泛型的“联合-查找”（Union-Find）数据结构。它在图论算法（如判断连通性、检测环、Kruskal最小生成树）和各类分组问题中扮演着核心角色。

*   **核心思想:** 用一片**逻辑上的“森林”**来表示集合划分。森林中的每一棵树都代表一个独立的集合，而树的**根节点**就是这个集合唯一的**“代表元”**。所有操作都围绕着维护这片森林的结构展开。

*   **核心成员:**
    *   `MyVector<size_t> _parent`: 核心数组，`_parent[i]` 存储索引 `i` 的父节点的索引。
    *   `MyVector<int> _rank`: 记录以索引 `i` 为根的树的“秩”（Rank），作为树高上界以指导合并。
    *   `size_t _count`: 当前独立集合（即森林中的树）的数量。
    *   **`MyHashMap<T, size_t> _to_index`**: **泛型实现的关键**。将用户提供的任意类型 `T` 映射到内部使用的、从0开始的连续整数索引。
    *   **`MyVector<T> _from_index`**: 与 `_to_index` 配对，用于从内部索引反向查询到原始的 `T` 类型数据。

*   **关键技术点:**
    1.  **泛型化与映射层 (Generalization & Mapping Layer):** 完美地解决了泛型 `T` 无法直接用作数组索引的问题。通过组合内部已实现的 `MyHashMap` 和 `MyVector`，构建了一个强大的双向映射层。这使得 `MyDisjointSet` 的核心算法可以继续在高效的、基于整数索引的 `_parent` 数组上运行，而对外则呈现出完全泛型的接口。这是**“组合优于继承”**和**组件复用**的绝佳实践。
    2.  **路径压缩 (Path Compression):** 在 `find` 操作中应用的、极为强大的“懒”优化。当从一个节点向上查找其根节点时，`_find_internal` 辅助函数会将这条路径上的**所有节点**的父指针，直接指向最终找到的根。这极大地“压平”了树的结构，使得后续对这些节点的查找操作几乎能一步到位。
    3.  **按秩合并 (Union by Rank):** 在 `unite` 操作中采用的、旨在保持树“矮胖”的启发式策略。在合并两棵树时，总是将**秩较低**（较矮）的树的根，连接到**秩较高**（较高）的树的根上。只有在两棵树的秩**完全相等**时，新形成的树的秩才会加一。此策略从根本上避免了树退化成低效的长链。
    4.  **摊还分析与近乎O(1)的效率:** 当“路径压缩”和“按秩合并”这两种优化**同时使用**时，`find` 和 `unite` 操作的平均（摊还）时间复杂度达到了 **O(α(n))**，其中 `α(n)` 是反阿克曼函数，其增长速度极其缓慢。在所有现实世界的应用场景中，可以认为其效率是**近乎常数时间**的，这是算法设计史上一个惊人的成果。

*   **核心洞察:** 并查集是**工程智慧**的结晶。它并没有像平衡树那样，通过复杂的操作（如旋转）来时时刻刻维持一种严格的结构不变性。相反，它采用了一种更务实的策略：**只维护一个“足够好”的、成本低廉的指标（秩），并辅以在“不得已而为之”（`find`操作）时顺手进行的、效果显著的优化（路径压缩）**。正是这种对“完美”的妥协，换来了无与伦比的实践效率。

```cpp
// 关键代码: _find_internal 的实现，完美展现了路径压缩的递归逻辑
